...
def _live_buffer_with_two_frames() -> List[Segment]:
    """Simulate how Live rewrites the same region multiple times."""
    control = Control(
    ...
    return [
        Segment("old-one", Style(color="red")),
        Segment("\n"),
        Segment("old-two", Style(color="yellow")),
        control,
        Segment("new-one", Style(color="green")),
        Segment("\n"),
        Segment("new-two", Style(color="green")),
    ]
...
def test_export_svg_final_only_uses_last_frame() -> None:
    console = Console(record=True, width=40)
    console._record_buffer[:] = _live_buffer_with_two_frames()

    svg_all = console.export_svg(clear=False)
    assert "old-one" in svg_all

    svg_final = console.export_svg(final_only=True, clear=True)
    assert "old-one" not in svg_final
    assert "new-two" in svg_final


def test_save_svg_final_only(tmp_path: pathlib.Path) -> None:
    console = Console(record=True, width=40)
    console._record_buffer[:] = _live_buffer_with_two_frames()
    export_path = tmp_path / "live.svg"
    console.save_svg(str(export_path), final_only=True)
    exported = export_path.read_text(encoding="utf-8")
    assert "new-one" in exported
    assert "old-two" not in exported
